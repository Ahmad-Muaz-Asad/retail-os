...
import { useAllSales } from "@/lib/db/hooks";
import type { SaleDocument } from "@/lib/db/schemas";

function SaleHistoryModal({
    sale,
    onClose,
}: {
    sale: SaleDocument;
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[80vh]">
                <div className="flex justify-between items-center p-4 border-b border-neutral-200 dark:border-neutral-800">
                    <h3 className="font-bold text-lg text-neutral-900 dark:text-neutral-50">Invoice #{sale.id.slice(-6).toUpperCase()}</h3>
                ...
                <div className="flex-1 overflow-y-auto p-4">
                    <div className="mb-4">
                        <p className="text-xs text-neutral-500">Date: {new Date(sale.createdAt).toLocaleString()}</p>
                        <p className="text-xs text-neutral-500">Cashier ID: {sale.employeeId || "Unknown"}</p>
                        {sale.customerId && <p className="text-xs text-neutral-500">Customer ID: {sale.customerId}</p>}
                    </div>
                        ...
                        <tbody>
                            {sale.items.map((item, idx) => (
                                <tr key={idx} className="border-b border-neutral-100 dark:border-neutral-800/50">
                                    ...
                                    <td className="py-2 text-right font-mono">Rs. {Math.round(item.price * (1 - item.discount / 100))}</td>
                                    <td className="py-2 text-right font-mono">Rs. {Math.round(item.price * (1 - item.discount / 100)) * item.quantity}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                    <div className="space-y-1 text-right text-sm border-t border-neutral-200 dark:border-neutral-800 pt-2">
                        <p className="text-neutral-500">Subtotal: Rs. {sale.subtotal}</p>
                        {sale.tax > 0 && <p className="text-neutral-500">Tax: Rs. {sale.tax}</p>}
                        {sale.discount > 0 && <p className="text-emerald-500">Discount: - Rs. {sale.discount}</p>}
                        <p className="font-bold text-lg mt-2 text-indigo-600 dark:text-indigo-400">Total: Rs. {sale.totalAmount}</p>
                    </div>
                </div>
            </div>
        </div>
...
export default function SalesPage() {
    ...
    const [selectedSale, setSelectedSale] = useState<SaleDocument | null>(null);
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
                            {sales.length} local receipt{sales.length !== 1 ? "s" : ""} recorded
                        </p>
                    </div>
                </div>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800">
                                        ...
                                        <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-neutral-500">Method</th>
                                        <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-neutral-500">Sync Status</th>
                                ...
                                <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                                    {sales.length === 0 && (
                                        <tr>
                                            <td colSpan={6} className="px-5 py-12 text-center text-sm text-neutral-400">No sales history found locally.</td>
                                        </tr>
                                    )}
                                    {sales.map(sale => (
                                        <tr key={sale.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                            <td className="px-5 py-3.5 font-bold text-neutral-900 dark:text-neutral-100 whitespace-nowrap">#{sale.id.slice(-6).toUpperCase()}</td>
                                            ...
                                            <td className="px-5 py-3.5 text-neutral-600 dark:text-neutral-400 whitespace-nowrap">
                                                <span className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-semibold tracking-wide ${sale.syncedAt ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30' : 'bg-amber-50 text-amber-700 dark:bg-amber-900/30'}`}>
                                                    {sale.syncedAt ? 'Synced' : 'Pending Sync'}
                                                </span>
                                            </td>
