...
export default function CustomersClient({ customers }: { customers: any[] }) {
    ...
    async function handleDelete(customer: any) {
        if (!window.confirm(`Delete customer "${customer.name}" (${customer.phone})?\n\nTheir past purchase history will be preserved.`)) return;
        ...
        <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-200 dark:border-neutral-800">
                    <h2 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">Customer Directory</h2>
                    <button
                ...
                {showAddForm && (
                    <form onSubmit={handleAddSubmit} className="px-5 py-4 border-b border-neutral-200 dark:border-neutral-800 bg-indigo-50/40 dark:bg-indigo-950/20">
                        {addError && (
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            <div className="flex-1">
                                <label htmlFor="cust-name" className="block text-xs font-medium text-neutral-700 dark:text-neutral-300 mb-1">Full Name</label>
                                <input
                                    required
                                    type="text"
                                    name="name"
                            ...
                            <div className="flex-1">
                                <label htmlFor="cust-phone" className="block text-xs font-medium text-neutral-700 dark:text-neutral-300 mb-1">Phone Number</label>
                                <input
                                    required
                                    type="text"
                                    name="phone"
                ...
                {customers.map(customer => (
                    ...
                    >
                        ...
                        <button
                            ...
                            className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity inline-flex items-center rounded-md bg-red-50 dark:bg-red-900/30 px-2 py-1 text-[10px] font-semibold text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/50 border border-red-200 dark:border-red-800 disabled:opacity-50"
                        ...
                        {/* Card content — clickable for history modal */}
                        <button
                        ...
                        >
                            ...
                            <p className="text-xs text-neutral-500">{customer.sales.length} orders total</p>
                        </button>
                    </div>
                ))}
            </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
                    <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[85vh]">
                        <div className="flex justify-between items-center p-4 border-b border-neutral-200 dark:border-neutral-800">
                            <div>
                                <h3 className="font-bold text-lg text-neutral-900 dark:text-neutral-50">{selectedCustomer.name}'s History</h3>
                                <p className="text-sm font-mono text-neutral-500">{selectedCustomer.phone}</p>
                            </div>
                            <button onClick={() => setSelectedCustomer(null)} className="text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-200">
                                <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                            </button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                            {selectedCustomer.sales.length === 0 ? (
                                <p className="text-sm text-neutral-500 text-center py-8">No order history available.</p>
                            ) : (
                                selectedCustomer.sales.map((sale: any) => (
                                    <div key={sale.id} className="border border-neutral-200 dark:border-neutral-800 rounded-xl p-4 bg-neutral-50 dark:bg-neutral-800/20">
                                        <div className="flex justify-between items-center mb-3">
                                            <div>
                                                <p className="font-bold text-sm text-neutral-900 dark:text-neutral-100">Invoice #{sale.id}</p>
                                                <p className="text-xs text-neutral-500">{new Date(sale.createdAt).toLocaleString()} • {sale.employee?.name || 'Cashier'}</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="font-bold text-indigo-600 dark:text-indigo-400 font-mono">Rs. {sale.totalAmount}</p>
                                                <p className="text-[10px] uppercase font-semibold text-neutral-500">{sale.paymentMethod} • {sale.status}</p>
                                            </div>
                                        </div>
                                        <div className="bg-white dark:bg-neutral-900 rounded-lg p-3 border border-neutral-100 dark:border-neutral-800 text-xs">
                                            {sale.items.map((item: any) => (
                                                <div key={item.id} className="flex justify-between py-1 border-b border-neutral-100 dark:border-neutral-800 last:border-0">
                                                    <span className="text-neutral-700 dark:text-neutral-300">{item.quantity}x {item.product?.name || "Unknown product"}</span>
                                                    <span className="font-mono text-neutral-600 dark:text-neutral-400">Rs. {item.priceAtSale * item.quantity}</span>
                                                </div>
                                            ))}
                                            {(sale.tax > 0 || sale.discount > 0) && (
                                                <div className="mt-2 pt-2 border-t border-neutral-100 dark:border-neutral-800 font-mono text-neutral-500">
                                                    {sale.tax > 0 && <div className="flex justify-between"><span>Tax</span><span>Rs. {sale.tax}</span></div>}
                                                    {sale.discount > 0 && <div className="flex justify-between text-emerald-500"><span>Discount</span><span>- Rs. {sale.discount}</span></div>}
                                                </div>
                                            )}
                                        </div>
                                    </div>
