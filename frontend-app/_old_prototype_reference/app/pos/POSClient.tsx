...
import { processCheckout, lookupCustomer, getProducts, getHeldCarts, searchCustomers, CartItem } from "./actions";
...
export default function POSClient({ products }: Props) {
        ...
        setCart((prev) => {
            ...
            return [
                ...prev,
                {
                    ...
                    price: product.price,
                    quantity: 1,
                    discount: product.defaultDiscount, // seed from product's default
                },
            ];
        });
    };
    ...
    const setItemDiscount = (productId: number, pct: number) => {
        setCart((prev) =>
            prev.map((c) =>
    ...
    const handleSelectSuggestion = async (suggestion: Customer) => {
        ...
        const res = await lookupCustomer(suggestion.phone);
        setIsLookingUp(false);
        if (res) {
            setCustomer(res);
            setDiscount(0);
            showToast("success", `Customer ${res.name} found!`);
        } else {
    ...
    const handleAction = (status: "COMPLETED" | "HOLD") => {
        startTransition(async () => {
            ...
            if (result.success) {
                ...
                showToast("success", status === "HOLD" ? "Cart placed on hold!" : "Sale recorded successfully!");
            } else {
    ...
    const handleResumeCart = (cartRecord: any) => {
        setCart(
            cartRecord.items.map((i: any) => ({
                ...
                price: i.priceAtSale,
                quantity: i.quantity,
                discount: 0, // held carts have price already baked in
            }))
        );
        ...
        setDiscount(cartRecord.discount);
        setPaymentMethod(cartRecord.paymentMethod as PaymentMethod);
        setActiveCartId(cartRecord.id);
        setShowHeld(false);
